...
and following packages will be imported by default:
* models._
* controllers._
* play.utils._

p(note). This means that you will need to have at least one class in each of these packages, otherwise the template ca not be compiled
...
p(note). It's worth noting that by default the precompiler is scanning only the first 20 lines of each template for variable declaration (<%@ var %>
