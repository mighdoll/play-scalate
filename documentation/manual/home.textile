...
(if you have any other version installed from scala or scalate than 0.1, then you will need to explicitly reference both ie. --with scalate-0.3,scala-0.1)
...
bc. object Application extends ScalateController  {
    def index(name: String = "Guest user") = render(name)  //renderArgs("name",name) also works
}
